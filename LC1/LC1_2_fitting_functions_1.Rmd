...
setwd("~/Brein_en_Cognitie/LC1/") # set to your working dir with the files you need.
```
...
noise = runif(length(sim_data), -1.5, 1.5)
sim_data = noise + sim_data
...
```{r}
fit_linear <- function(data, v){
  return(v*data$x)
}

fit_polynomial <- function(data, v){
  x<-data$x
  return(v[1] + v[2]*x + v[3]*x^2 + v[4]*x^3 + v[5]*x^4)
}
dat_short$error1<-fit_linear(dat_short,results1$par)
```
...
# ANSWER
results1<-optim(c(0),linear,data=dat_short,method="Brent", upper=10, lower=-10)
results2<-optim(c(-5,9,-4,1,.01),polynomial,data=dat_short,method="Nelder-Mead")

dat_short$error1<-fit_linear(dat_short,results1$par)
dat_short$error2<-fit_polynomial(dat_short,results2$par)
...
# to do so you need to invoke the linear function b1*x and use the estimated b1
# so in this case: results1$par[1]*dat_short$x
#lines(dat_short$x,dat_short$error1,col="green",lwd=2)
lines(dat_short$x,dat_short$error2,col="blue",lwd=2)
# here we add segments that indicate the distance or error from the model to the data:  
#segments(dat_short$x,dat_short$y,dat_short$x,dat_short$error1, col="red",lwd=2)
segments(dat_short$x,dat_short$y,dat_short$x,dat_short$error2, col="orange",lwd=2)
```
* Compare the MSE of the best fit for each model and indicate which is the best fitting one. Why do you think did the best fitting model won?
The best fitting plot was the polynomial function, as it can more accurately approach the relation between the points
...
```{r}
dat_full$error1<-fit_linear(dat_full,results1$par)
dat_full$error2<-fit_polynomial(dat_full,results2$par)
results1$value
results2$value

# simply use the plot() function to plot the true data
plot(dat_full$x,dat_full$y,ylim = c(-2,12))
# add a line with lines() that corresponds with the predicted y_hat.
# to do so you need to invoke the linear function b1*x and use the estimated b1
# so in this case: results1$par[1]*dat_full$x
lines(dat_full$x,dat_full$error1,col="green",lwd=2)
lines(dat_full$x,dat_full$error2,col="blue",lwd=2)
# here we add segments that indicate the distance or error from the model to the data:
segments(dat_full$x,dat_full$y,dat_full$x,dat_full$error1, col="red",lwd=2)
segments(dat_full$x,dat_full$y,dat_full$x,dat_full$error2, col="orange",lwd=2)
```
As we can see, the polynomial function overfitted onto the training set, which caused it to be unable to accurately predict the values outside the training set. The linear model is therefore a better fit.
...
where Q(t) is savings at time t and u1 and a1 are parameters.
...
ebbinghaus<- function(data,v){
    time = data$Interval_s/60
    u1 = v[1]
    a1 = v[2]
    y_hat = (c(u1 / (log10(time)^a1 + u1)))
...
simple_exponential<- function(data,v){
    time = data$Interval_s/60
    y_hat = (c((1 + v[1] * time)^-v[2]))
    error = y_hat-data$Ebbinghaus
    MSE = mean(error^2)
    return (MSE)
}

fit_ebbinghaus <- function(data, v){
  time = data$Interval_s/60
  return (c(v[1]/(log10(time)^v[2] + v[1])))
}

fit_simple_exponential <- function(data, v){
  time = data$Interval_s/60
  return (c((1 + v[1] * time)^-v[2]))
}
```
...
# Fit
results_ebbing1 <- optim(c(2.0, 1.0), ebbinghaus, data=ebbing_data, method="L-BFGS-B", upper=10, lower=-10)
results_ebbing2 <- optim(c(2.0, 1.0), simple_exponential, data=ebbing_data, method = "L-BFGS-B", upper=10, lower=-10)

# Estimate after fit
ebbing_data$error1 = fit_ebbinghaus(ebbing_data, results_ebbing1$par)
ebbing_data$error2 = fit_simple_exponential(ebbing_data, results_ebbing2$par)

# Plot ebbinhaus function
plot(ebbing_data$Interval_s/60, ebbing_data$Ebbinghaus)

lines(ebbing_data$Interval_s/60, ebbing_data$error1, col="green", lwd=2)

segments(ebbing_data$Interval_s/60, ebbing_data$Ebbinghaus, ebbing_data$Interval_s/60, ebbing_data$error1, col="red", lwd=2)
```

```{r}
# plot simple ebbinhaus function
plot(ebbing_data$Interval_s/60, ebbing_data$Ebbinghaus)
lines(ebbing_data$Interval_s/60, ebbing_data$error2, col="blue", lwd=2)
segments(ebbing_data$Interval_s/60, ebbing_data$Ebbinghaus, ebbing_data$Interval_s/60, ebbing_data$error2, col="yellow", lwd=2)
```

# As can be seen in the plots, both functions are nearly indentical. The errors don't seem to vary that much
...
result1<-optim(c(10, -10), fit_eggholder, method="L-BFGS-B", upper=10, lower=-10)
result2<-optim(c(-500.0, -400.0), fit_eggholder, method="L-BFGS-B", upper=-400, lower=-500)
result3<-optim(c(-230.0, -290.0), fit_eggholder, method="L-BFGS-B", upper=-230, lower=-290)
result4<-optim(c(70.0, 170.0), fit_eggholder, method="L-BFGS-B", upper=170, lower=70)
result5<-optim(c(300.0, 200.0), fit_eggholder, method="L-BFGS-B", upper=300, lower=200)
result1$value
result2$value
result3$value
result4$value
result5$value
```
The eggholder function has a lot of local minima, it is therefore very difficult to find the global minimum using these estimation methods